...
def detect_defects(text):
    ...
    This is a simple rule-based approach that can be enhanced with ML models
    """
    defect_keywords = {
...
def view_analysis(analysis_id):
    """View detailed analysis results by ID"""
    try:
        conn = sqlite3.connect('defect_analysis.db')
        cursor = conn.cursor()
        ...
        defect_summary = json.loads(analysis[4]) if analysis[4] else {}
        defects = json.loads(analysis[5]) if analysis[5] else []
        
        return render_template('analysis_detail.html',
                             analysis_id=analysis[0],
                             filename=analysis[1],
                             timestamp=analysis[2],
                             total_defects=analysis[3],
                             summary=defect_summary,
                             defects=defects)
                             
    except Exception as e:
        print(f"Analysis view error: {e}")
        flash('Error loading analysis details')
        return redirect(url_for('dashboard'))
